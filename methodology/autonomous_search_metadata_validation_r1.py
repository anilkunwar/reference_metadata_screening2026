# app.py - BibTeX Hallucination Validator (Full Expanded Version)
# ============================================================================
# This code is complete, unredacted, and contains all necessary error handling.
# ============================================================================
...
def safe_string_slice(value: Any, max_length: int = 50) -> str:
    """Safely slice string values, handling None and lists."""
    if value is None:
        return "Untitled"
...
def normalize_text(text: Optional[str]) -> str:
    """Normalize text for comparison (lowercase, strip symbols)."""
    if text is None:
        return ""
...
def format_authors_for_bibtex(authors: Union[str, List[str], None]) -> str:
    """Format authors list into BibTeX 'and' separated format."""
    if authors is None:
        return ""
...
def fetch_crossref_metadata(doi: str, timeout: int = DEFAULT_TIMEOUT) -> Optional[Dict[str, Any]]:
    ...
    # Extract Authors - FIXED SYNTAX ERROR HERE
    authors_data = msg.get("author", [])
    if authors_data and isinstance(authors_data, list):
        for author in authors_data:  # <-- Corrected from 'authors_' to 'authors_data:'
...
def compare_metadata_fields(original: Dict[str, Any], verified: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
    """Compare original entry with verified metadata."""
    comparison_results = {}
...
def merge_metadata_entries(original: Dict[str, Any], verified: Dict[str, Any], 
    ...
    """Merge original and verified data."""
...
def initialize_llm_pipeline(model_name: str, device: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            device_map="auto" if dev=="cuda" and torch.cuda.device_count()>1 else None
        )
...
def generate_bibtex_entry_string(entry: Dict[str, Any], cite_key: Optional[str] = None) -> str:
    """Generate BibTeX string for one entry."""
